...
\begin{tabular}{c} \psfig{file=./figures/fig2a.pdf,width=2.5in,height=2.5in,clip=}  \end{tabular}
 &  \begin{tabular}{c}
...
mod<-CNVassoc(quanti ~ CNV.2 + cov, family = "gaussian", data = dataMLPA, model = 'add', emsteps=10)
mod
@
...
<<echo=T>>=
pvals<-multiCNVassoc(probsRegions, formula="pheno~CNV",
model="mult", num.copies=0:2, cnv.tol=0.01)
@
...
fit.cnvtools <- CNVtest.binary(signal=ldf.signal,sample=sample,batch=batches,disease.status=trait,ncomp=ncomp,n.H0=3,n.H1=1,model.var="~cn")
chi.cnvtools  <-  -2*(fit.cnvtools$model.H0$lnL-fit.cnvtools$model.H1$lnL)
@

This results in a $\chi^2 =$ \Sexpr{round(CNVtest(fit,"LRT")$stat,2)} which does not differ greatly from the one given 
in CNVtools vignette \cite{CNVtools-vignette} (\Sexpr{round(chi.cnvtools,2)}), neither being statistically significant. \\
...
<<echo=FALSE>>=
fit.cnvtools <- CNVtest.binary(signal=ldf.signal,sample=sample,batch=batches,disease.status=trait,ncomp=3,n.H0=3,n.H1=1,model.disease="~as.factor(cn)")
chi.cnvtools  <-  -2*(fit.cnvtools$model.H0$lnL-fit.cnvtools$model.H1$lnL)
@

\noindent a $\chi^2$ of \Sexpr{round(CNVtest(fit,"LRT")$stat,2)} is obtained, similar to that in CNVtools-vignette \cite{CNVtools-vignette} 
(\Sexpr{round(chi.cnvtools,2)}). Again, neither is statistically significant.
...
%% falta posar alguna cosa sobre que tamb� es pot analitzar el CANARY data format ???....
...
dsim<-simCNVdataPois(n=4000,mu.surrog=0:2,sd.surrog=rep(0.4,3),w=c(0.25,0.5,0.25),lambda=lambda)
head(dsim)
@
...
table.gold<-c(exp(c(coef(fit.gold)[2],confint(fit.gold)[2,])),coef(summary(fit.gold))[2,4])
...
table.naive<-c(exp(c(coef(fit.naive)[2],confint(fit.naive)[2,])),coef(summary(fit.naive))[2,4])
